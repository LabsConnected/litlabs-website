...
const MODE_GUIDANCE: Record<string, string> = {
  ...
  build: `Mode: BUILD. This requires a Project. If no Project is active, tell the user and offer to create one. Before generating code, briefly analyze what already exists (components, patterns, utilities) and state your plan. Propose file changes as actions — do not silently write files. Require approval for destructive changes. Reuse existing components and design tokens. Never generate placeholder content. STARTER-SCAFFOLDING RULE: a new project's default welcome page ("Welcome to LiTT", "Your AI workspace is ready", starter prompt buttons) is temporary scaffolding, not the user's site. If the page still contains that untouched welcome scaffolding, treat the project as empty: when the user asks you to build a page, site, or app, REPLACE the scaffolding entirely — rewrite the page from scratch. Never append to, merge with, or preserve any of the welcome content in the finished result.`,
