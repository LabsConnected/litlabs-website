...
export type MediaRequest = {
  ...
  requestId?: string;
  /** Explicitly reclaim a failed durable operation with the same identity. */
  retry?: boolean;
};
...
// ── Shared authenticated implementation ──────────────────────────

/**
 * Server-side image generation implementation. Callers must authenticate
 * before invoking this function; it deliberately accepts a user identity
 * instead of forwarding browser cookies through an internal HTTP request.
 */
export async function generateMediaForUser(userId: string, body: MediaRequest) {
  ...
  // ── Durable claim before provider invocation ───────────────────
  // The unique user+requestId key is claimed while the operation is still
  // generating. A concurrent caller must replay or wait; it may not invoke
    ...
    return NextResponse.json(
      {
        ...
        error: "Unable to establish durable generation ownership",
        retryable: true,
      } satisfies GenerationErrorResponse,
    ...
    return NextResponse.json(
      {
        success: false,
        requestId,
        providerId: existing?.provider as MediaProviderId ?? providerId,
        code: isFailed ? "GENERATION_FAILED_RETRY_REQUIRED" : "GENERATION_IN_PROGRESS",
        error: isFailed
          ? "The previous generation failed. Retry explicitly with the same requestId."
          : "A generation with this requestId is already in progress",
        retryable: true,
      } satisfies GenerationErrorResponse,
  ...
  const claimedJobId = claim.job.id;
  ...
  if (!result || lastError) {
    ...
    const errorMsg = isQuota
      ...
      : rawMsg;
    await failGenerationJob(claimedJobId, errorMsg);
  ...
  // ── Persist and complete the durable claimed job ─────────────────
  // The job was claimed before provider invocation, so this completion uses
  // the same stable operation identity and cannot create a duplicate row.
  const generationJobId = claimedJobId;
  let assetPersistenceFailed = false;
  try {
    await updateGenerationJobMetadata(generationJobId, {
      durableUrl,
      durationMs: Date.now() - startTime,
    });
    await completeGenerationJob(generationJobId, `generation_job:${generationJobId}`, usedCostResult.providerCostCents);
  } catch (persistErr) {
    // Generation succeeded but Asset Lake persistence failed.
    // Do NOT fabricate an assetId or invoke the provider again.
    console.error(
