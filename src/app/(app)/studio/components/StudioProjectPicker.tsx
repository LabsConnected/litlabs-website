  ...
  return (
    <div className="relative hidden sm:block">
      ...
      {open && <div className="absolute left-0 top-full z-50 mt-1 w-64 rounded-xl border p-1.5 shadow-2xl" style={{ borderColor: "var(--studio-border-strong)", backgroundColor: "var(--studio-elevated)" }} role="listbox" aria-label="Projects">
        {loading ? <div className="px-2.5 py-3 text-[12px]" style={{ color: "var(--text-muted)" }}>Loading projects…</div> : error ? <div className="px-2.5 py-3 text-[12px]" style={{ color: "#fca5a5" }}>{error}</div> : projects.length === 0 ? <div className="px-2.5 py-3 text-[12px]" style={{ color: "var(--text-muted)" }}>No projects available.</div> : projects.map((project) => <button key={project.id} type="button" role="option" aria-selected={project.id === projectId} onClick={() => { onSelect(project.id); setOpen(false); }} className="flex min-h-11 w-full items-center gap-2 rounded-lg px-2.5 text-left transition hover:bg-white/8" style={{ color: project.id === projectId ? "var(--litt-primary)" : "var(--text-secondary)" }}><Folder size={13} className="shrink-0" /><span className="min-w-0 flex-1 truncate text-[12px] font-bold">{project.name}</span><span className="shrink-0 text-[11px]" style={{ color: "var(--text-muted)" }}>{project.sourceType ?? "project"}</span></button>)}
      </div>}
    </div>
  );
}
