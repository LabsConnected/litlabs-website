...
export default function RetroPlayerPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!active) return;
        if (!record)
          throw new Error("This game is not stored in this browser.");
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      if (
        data &&
        data.source === "ejs" &&
        (data.type === "error" || data.type === "ready")
      ) {
  ...
  async function pickBios(file?: File) {
    ...
    try {
      ...
      setEmulatorError(
        reason instanceof Error ? reason.message : "Could not read BIOS file.",
      );
    }
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#050507] text-white">
        <div className="text-center">
          <Gamepad2
            className="mx-auto animate-pulse text-fuchsia-400"
            size={40}
          />
          <p className="mt-4 text-sm font-bold text-white/50">
            Loading local cartridge…
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex min-h-screen items-center justify-center bg-[#050507] p-6 text-white">
        <div className="max-w-md rounded-3xl border border-white/10 bg-white/[.035] p-8 text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-white/45">
            {error ?? "This local game is unavailable."} ROMs are
            device-specific, so add it again from this browser if needed.
          </p>
          <Link
            href="/games/retro"
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-[11px] text-white/35">
                LiTT Retro Arcade · local session
                {isSatellaview && " · Satellaview / BS‑X"}
              </p>
            </div>
          </div>
          <button
        ...
        <aside className="space-y-4">
          {isSatellaview && (
            <section className="rounded-2xl border border-cyan-400/20 bg-cyan-400/[.05] p-5">
              ...
              <p className="mt-2 text-xs leading-5 text-white/55">
                Satellaview titles need the BS‑X BIOS to boot. Pick a copy here
                — it stays in this browser only, never uploaded.
              </p>
              {biosName ? (
          ...
          <section className="relative overflow-hidden rounded-2xl border border-violet-400/20 bg-gradient-to-b from-violet-500/10 to-transparent p-5">
            ...
            <h2 className="mt-2 text-lg font-black">
              {emulatorError ? "Emulator trouble." : "Chapter loaded."}
            </h2>
            <p className="mt-2 text-sm leading-6 text-white/45">
              {emulatorError
                ? emulatorError
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[.03] p-5">
            ...
            <dl className="mt-4 space-y-3 text-xs">
              ...
              <div className="flex justify-between gap-3">
                ...
                <dd className="text-right font-bold">{emulatorCore}</dd>
              </div>
              {isSatellaview && (
                <div className="flex justify-between gap-3">
                  <dt className="text-white/35">Mode</dt>
                  <dd className="text-right font-bold text-cyan-200">
                    Satellaview / BS‑X
                  </dd>
                </div>
              )}
          ...
          <section className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[.04] p-5">
            ...
            <p className="mt-2 text-xs leading-5 text-white/40">
              The ROM and BIOS were loaded from this browser only — the data
              stays in memory as a base64 URL inside the emulator iframe. LiTT
