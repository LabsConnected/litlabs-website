...
function TerminalStrip({ collapsed, onCollapseAction, onOpenTerminalAction, style }: { collapsed: boolean; onCollapseAction: () => void; onOpenTerminalAction: () => void; style?: CSSProperties }) {
  return <div className={`${styles.terminalStrip} ${collapsed ? styles.terminalCollapsed : ""}`} style={style} data-deck-panel="terminal"><div className={styles.terminalTabs}><button>Terminal</button><button>Problems</button><button>Output</button><button>Tests</button></div>{!collapsed && <div className={styles.terminalPlaceholder}>Terminal session not started. Select a project and open an authenticated session.</div>}<div className="flex gap-1"><button className={styles.iconButton} onClick={onCollapseAction} aria-label={`${collapsed ? "Expand" : "Collapse"} terminal`} title={`${collapsed ? "Expand" : "Collapse"} terminal`}>{collapsed ? "+" : "−"}</button><button className={styles.button} onClick={onOpenTerminalAction}><Terminal size={12} /> Open</button></div></div>;
}
