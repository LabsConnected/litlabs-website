...
const TerminalTool = forwardRef<TerminalToolHandle, TerminalToolProps>(
  function TerminalTool({ initialMode = "local", onOutput, onSessionChange, onStatusChange }, ref) {
  ...
  const connectRemote = useCallback(async () => {
    ...
    try {
      ...
      socket.on("connect", () => {
        termRef.current?.writeln(`\x1b[33mVerifying PTY at ${url}…\x1b[0m`);
        if (termRef.current) {
      ...
      socket.on("session:ready", (payload: { sessionId?: string }) => {
        if (payload?.sessionId) {
          ...
          termRef.current?.writeln(`\x1b[32m✓ Real PTY connected\x1b[0m`);
  ...
  useImperativeHandle(ref, () => ({
    runCommand(command: string) {
      ...
      if (!term || modeRef.current !== "remote" || status !== "connected" || !socket?.connected || !sessionIdRef.current) {
        return Promise.resolve({ command, output: "", exitCode: null, durationMs: 0, error: "The embedded shell is open, but the real PTY is not connected. Start `pnpm terminal:dev`, then connect the PTY." });
      }
      term.writeln(`\r\n\x1b[36m$ ${command}\x1b[0m`);
      return new Promise<TerminalCommandResult>((resolve) => {
        socket.timeout(305_000).emit("terminal:execute", { command }, (err: Error | null, result?: TerminalCommandResult) => {
          const completed = err || !result
            ? { command, output: "", exitCode: null, durationMs: 0, error: err?.message || "Terminal execution failed" }
            : result;
          if (completed.output) term.write(completed.output.replace(/\n/g, "\r\n"));
          if (completed.error) term.writeln(`\r\n\x1b[31m${completed.error}\x1b[0m`);
          onOutputRef.current?.(completed.output || completed.error || "");
          resolve(completed);
        });
      });
    },
