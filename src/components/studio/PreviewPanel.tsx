    ...
    <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
      ...
      <div className="grid h-32 place-items-center rounded-xl border border-dashed border-white/10 text-white/30">
        <div className="text-center">
          ...
          <p className="text-[10px]">{previewUrl ? `Preview: ${status ?? "ready"}` : "No preview running"}</p>
          {previewUrl ? (
            <a href={previewUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex rounded-lg border border-white/10 px-3 py-1 text-[10px] text-white/70 hover:text-white">
              Open preview
            </a>
          ) : null}
        </div>
