...
export default function LiTTTerminal() {
  ...
  const [logs, setLogs] = useState<LogEntry[]>(() =>
    buildBootLogs(profile.displayName || "Member"),
  );
  ...
  const intentionalStopRef = useRef(false); // Track if user intentionally stopped recording
      ...
      switch (cmd.toLowerCase()) {
        ...
        case "continuous":
          setContinuousMode((v) => {
            ...
            addLog({
              type: "success",
              text: `Continuous listening ${next ? "ENABLED" : "DISABLED"}.\n${next ? "Mic will auto-restart after each response." : "Mic stops after each response."}`,
            });
            return next;
          });
          setIsProcessing(false);
        ...
        case "wake":
          setWakeWordEnabled((v) => {
            ...
            addLog({
              type: "success",
              text: `Wake word detection ${next ? "ENABLED" : "DISABLED"}.\n${next ? "Say 'Hey LiTT' or 'LiTT' to activate." : "All speech will be processed."}`,
            });
            return next;
          });
          setIsProcessing(false);
  ...
  useEffect(() => {
    ...
    // Filters out low-confidence and very short transcriptions (likely noise)
    ...
    rec.onresult = (event: SpeechRecognitionEvent) => {
      ...
      if (lastResult.isFinal) {
        // Check if user intentionally stopped (push-to-talk mode)
        ...
        // Skip if not meaningful (too short or low confidence)
        if (!isMeaningfulTranscript(transcript, confidence)) {
          addLog({
            type: "system",
            text: "Ignored low-confidence or incomplete audio input.",
          });
          setInput("");
          return;
        }
      ...
      >
        ...
        <div className="flex items-center gap-3 sm:gap-5 text-[10px] font-mono">
          ...
          <button
            ...
            className="transition-opacity hover:opacity-100"
            style={{ opacity: continuousMode ? 1 : 0.4 }}
            title={continuousMode ? "Continuous mode ON" : "Continuous mode OFF"}
          >
            <span
              className={`text-[10px] font-mono font-bold ${continuousMode ? "text-[#00ff9d]" : "text-gray-500"}`}
            >
              CONT
            </span>
          </button>
          ...
          <button
            ...
            className="transition-opacity hover:opacity-100"
            style={{ opacity: wakeWordEnabled ? 1 : 0.4 }}
            title={wakeWordEnabled ? "Wake word ON - Say 'Hey LiTT'" : "Wake word OFF"}
          >
            <span
              className={`text-[10px] font-mono font-bold ${wakeWordEnabled ? "text-[#ff0055]" : "text-gray-500"}`}
            >
              WAKE
            </span>
          </button>
