...
const STAGES: StageDef[] = [
  ...
  { id: "preview", label: "Preview", icon: Play, accent: "#b58cff", description: "See the result live as it's being built. No waiting." },
  ...
  { id: "launch", label: "Launch", icon: Rocket, accent: "#a8ff2f", description: "Deploy the finished project with one click. It goes live instantly." },
];
    ...
    <div className="w-full">
      <div role="tablist" aria-label="LiTTree Studio workflow stages" className="mb-6 flex flex-wrap gap-2">
        {STAGES.map((stage) => {
          ...
          return (
            <button key={stage.id} role="tab" aria-selected={isActive} aria-controls="demo-stage-panel" id={`demo-tab-${stage.id}`} onClick={() => setActive(stage.id)}
  ...
  switch (stage) {
    case "mission":
      return (
        <div className="space-y-3">
          <div className="rounded-xl border border-white/10 bg-black/30 p-4"><div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-white/30">User prompt</div><div className="text-sm text-white/80">Build me a premium website for my music.</div></div>
          <div className="rounded-xl border border-[#a8ff2f]/20 bg-[#a8ff2f]/5 p-4">
            ...
            <div className="mt-2 space-y-1 text-xs text-white/50"><div>&bull; Hero with artist image and latest release</div><div>&bull; Music player with track list</div><div>&bull; Tour dates and ticket links</div><div>&bull; Merch store integration</div><div>&bull; Mobile-responsive, SEO-optimized</div></div>
          </div>
        </div>
      );
    case "plan":
      return (
        <div className="space-y-2">
          {["Design hero section with artist image", "Build responsive layout (mobile-first)", "Add music player component", "Create tour dates section", "Link merch store", "Optimize and prepare deployment"].map((step, i) => (
      ...
      return (
        <div className="space-y-2">
          <div className="rounded-lg border border-white/8 bg-black/40 p-3 font-mono text-xs leading-5 text-white/60">
            ...
            <div className="text-white/40">  index.html &mdash; 4.2 KB</div><div className="text-white/40">  styles.css &mdash; 8.1 KB</div><div className="text-white/40">  player.js &mdash; 3.7 KB</div><div className="text-white/40">  assets/hero.jpg &mdash; 124 KB</div>
            <div className="mt-2 flex items-center gap-1.5 text-[#a8ff2f]"><Check size={11} /> All files created successfully</div>
          </div>
          <div className="grid grid-cols-3 gap-2">{["index.html", "styles.css", "player.js"].map((f) => (<div key={f} className="rounded-lg border border-white/8 bg-white/3 p-2.5"><FileCode size={14} className="mb-1.5 text-[#65f4ff]/60" /><div className="text-[10px] font-bold text-white/60">{f}</div></div>))}</div>
        </div>
      );
    case "preview":
      return (
        <div className="space-y-3">
          <div className="overflow-hidden rounded-lg border border-white/10 bg-gradient-to-b from-purple-950/30 to-black/50">
            <div className="p-5">
              <div className="mb-4 flex items-center justify-between"><div className="text-[10px] font-black uppercase tracking-wider text-[#b58cff]">Live Preview</div><div className="flex gap-1"><span className="h-2 w-2 rounded-full bg-[#b58cff]/40" /><span className="h-2 w-2 rounded-full bg-white/10" /><span className="h-2 w-2 rounded-full bg-white/10" /></div></div>
              <div className="mb-4 h-28 rounded-lg bg-gradient-to-br from-[#b58cff]/25 via-[#a8ff2f]/8 to-transparent" />
          ...
          <div className="text-xs text-white/40">Preview updates in real time as files change.</div>
        </div>
      );
    case "approval":
      return (
        <div className="space-y-3">
          <div className="rounded-xl border border-[#b58cff]/25 bg-[#b58cff]/5 p-5">
            ...
            <div className="mb-4 text-sm text-white/80">LiTT wants to deploy your music website to a public URL. This action will make the project visible to anyone with the link.</div>
            <div className="mb-4 rounded-lg border border-white/8 bg-black/30 p-3 text-xs text-white/50"><div className="mb-1 font-bold text-white/70">What will happen:</div><div>&bull; Build the production bundle</div><div>&bull; Deploy to the edge network</div><div>&bull; Generate a public URL</div></div>
            <div className="flex gap-2"><button className="rounded-lg bg-[#b58cff] px-5 py-2.5 text-xs font-black text-black transition hover:bg-[#c89dff]">Approve &amp; Deploy</button><button className="rounded-lg border border-white/15 px-5 py-2.5 text-xs font-bold text-white/50 transition hover:bg-white/5">Review changes first</button></div>
          </div>
    ...
    case "launch":
      return (
        <div className="space-y-3">
          <div className="rounded-lg border border-white/8 bg-black/40 p-3 font-mono text-xs leading-5 text-white/60">
            <div className="text-[#a8ff2f]">$ Deploying to production...</div>
            <div className="text-white/40">  Building bundle: 3 files, 0 errors</div><div className="text-white/40">  Uploading to edge: complete</div><div className="text-white/40">  DNS propagation: complete</div>
            <div className="mt-2 flex items-center gap-1.5"><Terminal size={11} className="text-[#a8ff2f]" /><span className="text-[#a8ff2f]">Deployment successful</span></div>
          </div>
          <div className="flex flex-col items-center rounded-xl border border-[#a8ff2f]/20 bg-[#a8ff2f]/5 py-6 text-center">
            ...
            <div className="text-xl font-black text-white">Your project is live.</div>
            <div className="mt-1 text-xs text-white/50">Share the URL with anyone.</div>
            <div className="mt-3 inline-flex items-center gap-2 rounded-lg border border-[#a8ff2f]/25 bg-[#a8ff2f]/8 px-4 py-2 text-xs font-bold text-[#a8ff2f]"><span className="h-1.5 w-1.5 rounded-full bg-[#a8ff2f]" />https://your-music.litlabs.net</div>
          </div>
        </div>
      );
    default: return null;
