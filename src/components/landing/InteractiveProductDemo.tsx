    ...
    <div className="w-full">
        ...
        {STAGES.map((stage, index) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-lg border px-4 py-2.5 text-xs font-black transition-colors duration-200 ${isActive ? "border-white/20 bg-white/8 text-white" : "border-white/8 bg-transparent text-white/55 hover:border-white/15 hover:text-white/70"}`}
      ...
      <div id="demo-stage-panel" role="tabpanel" aria-labelledby={`demo-tab-${active}`} className="overflow-hidden rounded-2xl border border-white/12 bg-[#0a0d14] shadow-[0_30px_80px_rgba(0,0,0,.5)]">
        <div className="flex items-center gap-2 border-b border-white/8 bg-[#0d1018] px-4 py-3">
          ...
          <div className="ml-3 flex min-w-0 items-center gap-2 text-xs font-bold text-white/55"><span className="relative h-5 w-5 shrink-0 overflow-hidden rounded"><Image src="/brand/litt-mascot-avatar.png" alt="LiTT" fill sizes="20px" className="object-cover" /></span><span className="truncate">LiTTree Studio &mdash; {current.label}</span></div>
          <div className="ml-auto flex items-center gap-1.5 text-[10px] font-bold text-white/50"><span className="h-1.5 w-1.5 rounded-full bg-[#a8ff2f]" />Connected</div>
        </div>
        ...
        <div className="flex min-h-[360px]">
          <div className="hidden w-48 shrink-0 border-r border-white/8 bg-[#080a10] p-3 sm:block">
            ...
            <div className="space-y-1">
              {[{ name: "mission.md", active: active === "mission" }, { name: "index.html", active: active === "build" || active === "preview" }, { name: "styles.css", active: active === "build" || active === "preview" }, { name: "player.js", active: active === "build" || active === "preview" }, { name: "assets/", active: active === "build" || active === "preview" }].map((file) => (
                <div key={file.name} className={`flex items-center gap-2 rounded-md px-2 py-1.5 text-xs ${file.active ? "bg-white/5 text-white/70" : "text-white/50"}`}>
                  {file.name.endsWith("/") ? <Folder size={12} className="shrink-0 text-[#65f4ff]/50" /> : <FileCode size={12} className="shrink-0 text-white/50" />}{file.name}
                </div>
              ))}
            </div>
            <div className="mt-4 border-t border-white/8 pt-3">
              <div className="mb-2 text-[10px] font-black uppercase tracking-wider text-white/50">History</div>
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 text-[10px] text-white/50"><GitBranch size={10} className="text-[#a8ff2f]/50" />Initial commit</div>
                {active !== "mission" && <div className="flex items-center gap-2 text-[10px] text-white/50"><GitBranch size={10} className="text-[#65f4ff]/50" />Checkpoint 1</div>}
                {(active === "launch" || active === "approval") && <div className="flex items-center gap-2 text-[10px] text-white/50"><GitBranch size={10} className="text-[#b58cff]/50" />Deploy checkpoint</div>}
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-5">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div><div className="text-sm font-black" style={{ color: current.accent }}>{current.label}</div><div className="text-xs text-white/55">{current.description}</div></div>
            </div>
    ...
    case "mission":
      return (
        <div className="space-y-3">
          <div className="rounded-xl border border-white/10 bg-black/30 p-4"><div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-white/50">User prompt</div><div className="text-sm text-white/80">Build a premium launch page for an independent music artist named After Midnight.</div></div>
      ...
      return (
        <div className="space-y-2">
          <div className="rounded-lg border border-white/8 bg-black/40 p-3 font-mono text-xs leading-5 text-white/60">
            ...
            <div className="text-white/55">  index.html &mdash; hero, bio, release section</div><div className="text-white/55">  styles.css &mdash; responsive layout, brand colors</div><div className="text-white/55">  assets/ &mdash; cover art, social images</div>
    ...
    case "preview":
      return (
        <div className="space-y-3">
          ...
          <div className="text-xs text-white/55">Preview updates as files change.</div>
        </div>
      );
    case "approval":
      return (
        <div className="space-y-3">
          ...
          <div className="text-xs text-white/55">LiTT never deploys, deletes, or performs sensitive actions without your approval.</div>
        </div>
      );
    case "launch":
      return (
        <div className="space-y-3">
          <div className="rounded-lg border border-white/8 bg-black/40 p-3 font-mono text-xs leading-5 text-white/60">
            ...
            <div className="text-white/55">  Building production bundle</div><div className="text-white/55">  Verifying build output</div><div className="text-white/55">  Ready for deployment</div>
