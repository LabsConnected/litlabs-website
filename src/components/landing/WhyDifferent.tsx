...
function MiniVisual({ type, accent }: { type: Differentiator["visual"]; accent: string }) {
  switch (type) {
    ...
    case "files": return (<div className="flex h-full items-center gap-1.5 px-4">{["index.html", "styles.css", "player.js", "assets/"].map((f) => (<div key={f} className="flex items-center gap-1 rounded border border-white/8 bg-white/3 px-2 py-1 text-[9px] font-bold text-white/55"><FileCode size={8} style={{ color: `${accent}80` }} />{f}</div>))}</div>);
    ...
    case "export": return (<div className="flex h-full items-center justify-center gap-2 px-4"><div className="rounded-lg border border-white/10 bg-white/3 px-3 py-2"><Download size={14} style={{ color: accent }} /></div><div className="text-[9px] font-bold text-white/55">Export all files</div></div>);
    case "launch": return (<div className="flex h-full items-center justify-center px-4"><div className="flex items-center gap-2 rounded-lg border px-3 py-2" style={{ borderColor: `${accent}25`, backgroundColor: `${accent}08` }}><Rocket size={12} style={{ color: accent }} /><span className="text-[9px] font-black" style={{ color: accent }}>READY TO SHIP</span><span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: accent }} /></div></div>);
    default: return null;
  }
}
