...
import { FolderOpen, Image as ImageIcon, FileText, Layers, Star, Search, Plus } from "lucide-react";
...
export default function AssetLibrary({ onAssetSelect }: AssetLibraryProps) {
  ...
  const [assets, setAssets] = useState<Asset[]>([
    { id: "1", name: "Hero Banner", type: "template", favorite: true, createdAt: new Date() },
    { id: "2", name: "Product Card", type: "template", createdAt: new Date() },
    { id: "3", name: "Logo v1", type: "image", favorite: true, createdAt: new Date() },
  ...
  const getAssetIcon = (type: Asset["type"]) => {
    switch (type) {
      case "image": return <span role="img" aria-label="Image"><ImageIcon size={16} /></span>;
      case "text": return <FileText size={16} />;
      case "shape": return <Layers size={16} />;
      case "scene": return <FolderOpen size={16} />;
      case "template": return <Star size={16} />;
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold" style={{ color: T.textColor }}>Asset Library</span>
        </div>
        <button
      ...
      <div className="flex items-center gap-2">
        {["all", "template", "image", "text", "shape", "scene"].map((type) => (
          <button
            ...
            style={{
              backgroundColor: filterType === type ? T.accentColor + "15" : "transparent",
              color: filterType === type ? T.accentColor : T.textMuted,
              border: filterType === type ? "1px solid " + T.accentColor + "30" : "transparent",
            }}
          >
            {type}
          </button>
        ...
        <button
          ...
          style={{
            backgroundColor: showFavorites ? T.accentColor + "15" : "transparent",
            color: showFavorites ? T.accentColor : T.textMuted,
            border: showFavorites ? "1px solid " + T.accentColor + "30" : "transparent",
          }}
        >
        ...
        {filteredAssets.map((asset) => (
          ...
          >
            <div className="flex items-start justify-between mb-1">
              <div className="flex items-center gap-1" style={{ color: T.accentColor }}>
              ...
              >
                <Star size={10} className={asset.favorite ? "fill-current" : ""} style={{ color: asset.favorite ? "#f59e0b" : T.textMuted }} />
              </button>
            </div>
      ...
      {filteredAssets.length === 0 && (
        <div className="text-center py-4">
          <FolderOpen size={24} className="mx-auto mb-2" style={{ color: T.textMuted }} />
          <p className="text-xs" style={{ color: T.textMuted }}>No assets found</p>
        </div>
      )}
    </div>
  );
