...
# Clerk reads NEXT_PUBLIC_CLERK_PROXY_URL in mergeNextClerkPropsWithEnv and
# passes it to ClerkProvider as proxyUrl. That runs on the client, so Next
# inlines the value at BUILD time — without this ARG the browser bundle ships
# proxyUrl undefined and never uses the /__clerk proxy that src/proxy.ts
# serves, no matter what the runtime environment holds.
...
ARG NEXT_PUBLIC_TERMINAL_HTTP_URL
ENV NEXT_PUBLIC_TERMINAL_HTTP_URL=$NEXT_PUBLIC_TERMINAL_HTTP_URL
ARG NEXT_PUBLIC_TERMINAL_WS_URL
ENV NEXT_PUBLIC_TERMINAL_WS_URL=$NEXT_PUBLIC_TERMINAL_WS_URL
