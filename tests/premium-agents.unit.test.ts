...
const mockQueryBuilder = {
  ...
  single: vi.fn(async () => ({ data: mockUser, error: mockSupabaseError })),
  maybeSingle: vi.fn(async () => {
    // Different callers use maybeSingle with different mock data
    // The caller is determined by the chain context — we use a simple
    // approach: return based on what was set
    return { data: null, error: null };
  }),
  order: vi.fn(() => mockQueryBuilder),
  limit: vi.fn(() => mockQueryBuilder),
  insert: vi.fn(async () => ({
    data: mockOrderInsert,
    error: null,
    select: vi.fn(() => ({
      single: vi.fn(async () => ({ data: mockOrderInsert, error: null })),
    })),
  })),
  update: vi.fn(async () => ({ error: mockOrderUpdateError })),
};

// Helper: create a chainable that resolves to the given result.
// Supports .eq().single(), .eq().maybeSingle(), .eq().eq().order().limit().maybeSingle(), etc.
...
describe("marketplace/agents/[id]/checkout", () => {
  ...
  it("sets product_type=agent in Stripe metadata", async () => {
    const res = await callCheckout("agent-uuid-001");
    expect(res.status).toBe(200);
    const body = parseBody(lastFetchBody);
  ...
  it("uses idempotency key based on order ID", async () => {
    const res = await callCheckout("agent-uuid-001");
    expect(res.status).toBe(200);
    const body = parseBody(lastFetchBody);
    expect(body.idempotency_key).toBe("marketplace_order_order-uuid-456");
  });
});
