...
- Keep canonical `main` clean and synchronized with the authoritative remote.
...
- Never overwrite unrelated user changes; never discard uncommitted work unless explicitly instructed.
- Never use destructive Git commands casually.
- Do not force-push `main`; do not reset canonical `main` to a feature branch.
- Do not merge partially validated work into `main`.
- Fetch the remote and check branch divergence before final integration.
