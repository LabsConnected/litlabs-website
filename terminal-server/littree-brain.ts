...
export async function askLiTTBrain(prompt: string) {
  const messages: ChatMessage[] = [
    {
      role: "system",
      content:
        "You are LiTT Brain, the AI operating layer for LiTTree Lab Studios. You help users build software. " +
...
export async function handleLiTTBrainCommand(input: string): Promise<string> {
  const args = input.trim().split(/\s+/).slice(1);
  const command = args[0]?.toLowerCase();
  const rest = args.slice(1).join(" ");

  const systemContext = `You are the LiTT Brain agent inside LiTTree OS Terminal.
...
`;
  ...
  return await askLiTTBrain(prompt);
}
