/**
 * LiT Discord Integration System
...
export async function sendDiscordMessage(
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...message,
        username: message.username || "LiT",
        avatar_url:
          message.avatar_url ||
          `${process.env.NEXT_PUBLIC_SITE_URL || "https://litlabs.net"}/lit-avatar.png`,
      }),
    });
...
export async function notifyDeployment(
  ...
  await sendDiscordMessage("agents", {
    ...
    avatar_url: `${process.env.NEXT_PUBLIC_SITE_URL || "https://litlabs.net"}/lit-avatar.png`,
    embeds: [
      {
