 ...
 *     command separator
 *   - optional `browser` targets a specific application (e.g. `chrome`, `brave`, `edge`).
 *
 ...
 */
export function buildWindowsStartCommand(url: string, browser?: string): string {
  const safeUrl = url.replace(/"/g, "");
  if (browser) {
    const safeBrowser = browser.replace(/"/g, "").trim();
    return `start "" "${safeBrowser}" "${safeUrl}"`;
  }
...
export function describeLaunch(
  ...
  const targetBrowser = browser ?? process.env.LITT_BROWSER;
  ...
  switch (kind) {
    ...
    case "windows":
      ...
      return { kind, command: buildWindowsStartCommand(url, targetBrowser), args: [], usesShell: true };
    case "macos":
      if (targetBrowser) {
        return { kind, command: "open", args: ["-a", targetBrowser, url], usesShell: false };
      }
      return { kind, command: "open", args: [url], usesShell: false };
    case "linux":
      if (targetBrowser) {
        return { kind, command: targetBrowser, args: [url], usesShell: false };
      }
...
/**
 * Open a URL in the system browser or a specified browser.
 *
 ...
 * @param browser Optional browser binary or name (e.g. 'chrome', 'brave', 'edge').
 *                Defaults to process.env.LITT_BROWSER if not passed.
 */
export async function openBrowser(
  ...
  if (platform === "win32") {
    try {
      await new Promise<void>((resolve, reject) => {
        deps.exec(buildWindowsStartCommand(url, targetBrowser), {
          windowsHide: true,
          timeout: 5000,
        }, (err) => {
          if (err) reject(err);
  ...
  // ─── WSL (Linux under Windows) Interop ──────────────────────────
  if (isWsl()) {
    // If a browser is specified or default Windows launcher is available,
    // invoke cmd.exe from WSL to launch the host browser cleanly.
    const cmdCmd = buildWindowsStartCommand(url, targetBrowser);
    try {
      await new Promise<void>((resolve, reject) => {
        deps.exec(`cmd.exe /c ${cmdCmd}`, { timeout: 5000 }, (err) => {
          if (err) reject(err);
          else resolve();
        });
      });
      ...
      // Fall through to standard Linux launchers
    }
  }
  ...
  if (platform === "darwin") {
    command = "open";
    args = targetBrowser ? ["-a", targetBrowser, url] : [url];
  } else {
    command = targetBrowser || "xdg-open";
    args = [url];
  }
